...
{
    public class ClaseCrudFactory : CrudFactory
    {
        private ClaseMapper mapper;
        private ClasesXClienteMapper mapperClaseXCliente;
        public ClaseCrudFactory() : base()
        {
            mapper = new ClaseMapper();
            mapperClaseXCliente = new ClasesXClienteMapper();
        ...
        public override int Create(BaseClass clase)
        {
            SqlOperation operation = mapper.GetCreateStatement(clase);
            return dao.ExecuteStoredProcedure(operation);
        }

        public int CreateInscripcion(BaseClass inscripcion)
        {
            SqlOperation operation = mapperClaseXCliente.GetCreateStatement(inscripcion);
            return dao.ExecuteStoredProcedure(operation);
        }
        ...
        public List<T> RetrieveClasesXCliente<T>(string cedula)
        {
            List<T> lstResults = new List<T>();
            List<Dictionary<string, object>> dataResults = dao.ExecuteStoredProcedureWithQuery(mapperClaseXCliente.GetRetrieveClasesXClienteStatement(cedula));

            if (dataResults.Count > 0)
            {
                var dtoObjects = mapperClaseXCliente.BuildGetObjects(dataResults); // check
        ...
        {
            SqlOperation operation = mapper.GetUpdateCupoStatement(idClase);
            return dao.ExecuteStoredProcedure(operation);
        }
