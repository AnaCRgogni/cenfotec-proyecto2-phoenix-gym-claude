...
{
    public class MensualidadCrudFactory :CrudFactory
    {
        private MensualidadMapper mapper;

        public MensualidadCrudFactory() : base()
        {
            mapper = new MensualidadMapper();
            dao = SqlDao.GetInstance();
        }

        public override int Update(BaseClass membresia)
        {
            SqlOperation operation = mapper.GetUpdateStatement(membresia);
            return dao.ExecuteStoredProcedure(operation);
        }

        public override int Create(BaseClass Mensualidad)
        {
            SqlOperation operation = mapper.GetCreateStatement((BaseClass)Mensualidad);
            return dao.ExecuteStoredProcedure(operation);
        }
        ...
        {
            SqlOperation operation = mapper.GetRetrieveMensualidad(cedula,fecha);
            return dao.ExecuteStoredProcedure(operation);
        }
